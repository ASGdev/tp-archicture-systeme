...
PRIVATE void trigger_clearing(){
	...
	for (i = 0; i < NR_FRAMES; i++)
	{
		...
		// verify pid (verify current proc)
...
int temp = CLEARING_INTERVAL;
PUBLIC void update_clearing_trigger(){
	if(temp == 0){
		trigger_clearing();
		//kprintf("[MM] Clearing triggered");
		temp = CLEARING_INTERVAL;
	} else {
		temp--;
	}
}
...
PRIVATE int allocf(void)
{
	...
	for (i = 0; i < NR_FRAMES; i++){
			...
			/* as there is no free page, we are going to get some candidates */
			if (frames[i].owner == curr_proc->pid)
			{
				/* Skip shared pages. */
				if (frames[i].count > 1)
				...
				// class 0
				if(!(pted->dirty & 1) && pted->accessed == 0){
					goto found;
				}

				// class 1 - need to swap as it was modified
				if(pted->dirty == 1 && pted->accessed == 0){
					goto found_need_swap;
				}

				// class 2
				if(pted->dirty == 0 && pted->accessed == 1){
					goto found;
				}

				// class 3 - need to swap
				if(pted->dirty == 1 && pted->accessed == 1){
					goto found_need_swap;
				}
			}
	}

	kprintf("[MM] Error : no candidate page was found!");
...
PUBLIC int pfault(addr_t addr)
{
	...
	kprintf("page fault at addr %x", addr);
