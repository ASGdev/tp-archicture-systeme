...
PRIVATE struct
{
	...
	unsigned counter_access; // used for near-LRU implementation in clock
...
/**
 * @brief function called in clock.c in do_clock for update the frame table
 * 
 * @param none
 * 
 * @returns none
...
PUBLIC void framesUpdate(){
	...
	{
		if(frames[i].count != 0){
			if(getpte(curr_proc,(frames[i].addr))->accessed == 1){
				frames[i].counter_access = frames[i].counter_access +1;
			}
		}
...
PRIVATE int allocf(void)
{
	...
	int lru; /* Least recently used page*/
	
	#define OLDEST(x, y) (frames[x].age < frames[y].age)
	#define LRU(x, y) (frames[x].counter_access < frames[y].counter_access)
	#define LRUEQ(x, y) (frames[x].counter_access = frames[y].counter_access)
