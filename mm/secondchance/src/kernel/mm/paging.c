...
PRIVATE struct
{
	...
	unsigned seconde_chance;
} frames[NR_FRAMES] = {{0, 0, 0, 0, 0},  };


int global_default_page = 0;
/**
...
PRIVATE int allocf(void)
{
	...
	for (i = 0; i < NR_FRAMES; i++)
	{
		...
		{	
			...
			{
				frames[i].seconde_chance = 1;
			}
		}
	}
	
	/*If we still have no frame, we take one anyway and 
	* too bad for the seconde chance algorithm.*/
	if(oldest < 0){
		for (i = 0; i < NR_FRAMES; i++)
		{
			/* Local page replacement policy. */
	...
	{
		kprintf("----Default Page :%d----",global_default_page++);
		//fflush(stdout);
		return (-1);
	}	
